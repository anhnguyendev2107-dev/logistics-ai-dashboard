...
export function ExplainPanel({
  ...
  return (
    <aside className="hidden h-full w-96 flex-shrink-0 flex-col border-l border-zinc-200/60 bg-white/70 backdrop-blur-md xl:flex dark:border-zinc-800/60 dark:bg-zinc-950/70">
      ...
      <div className="flex flex-1 flex-col gap-4 overflow-y-auto px-4 py-4 text-xs">
        ...
        {block.sample_rows.length > 0 ? (
          <Section title={`Sample rows (showing ${block.sample_rows.length} of ${block.rows_used})`}>
                ...
                <thead className="bg-zinc-50 text-zinc-500 dark:bg-zinc-900/50">
                  <tr>
                    {Object.keys(block.sample_rows[0]).map((k) => (
                      <th
                        key={k}
                        className="whitespace-nowrap px-2.5 py-1.5 text-left font-medium uppercase tracking-wider"
                      >
                        {k}
                      </th>
                    ))}
...
function Stat({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      <div className="text-[9px] font-semibold uppercase tracking-[0.08em] text-zinc-500 dark:text-zinc-400">
        {label}
      </div>
      <div
        className={cn(
